...
> ActionWorkflowJobsResponse list_admin_workflow_jobs(status=status, page=page, limit=limit)
...
with gitea_api.ApiClient(configuration) as api_client:
    ...
    try:
        # Lists all jobs
        api_response = api_instance.list_admin_workflow_jobs(status=status, page=page, limit=limit)
