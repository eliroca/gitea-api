...
class Commit(BaseModel):
    ...
    files: Optional[List[CommitAffectedFiles]] = Field(default=None, description="Files contains information about files affected by the commit")
    html_url: Optional[StrictStr] = Field(default=None, description="HTMLURL is the web URL for viewing the commit")
    parents: Optional[List[CommitMeta]] = Field(default=None, description="Parents contains the parent commit information")
    sha: Optional[StrictStr] = Field(default=None, description="SHA is the commit SHA hash")
    stats: Optional[CommitStats] = None
    url: Optional[StrictStr] = Field(default=None, description="URL is the API URL for the commit")
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.parents:
            for _item_parents in self.parents:
                _items.append(_item_parents.to_dict() if _item_parents is not None else None)
